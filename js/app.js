...
async function init() {
  try {
    ...
    document.getElementById('weather-panel').innerHTML =
      '<div style="padding:16px;color:#d46a6a;font-size:12px">Failed to load data files. Check that data/config.json and data/plants.json exist.</div>';
  }
}
...
function loadLogs() {
  try {
    ...
    state.plants.forEach(p => {
      const m = saved && saved.plantMutables
        ? saved.plantMutables.find(x => x.id === p.id)
        : null;
      p._notes       = m ? (m._notes       || [])   : [];
      p._feeds       = m ? (m._feeds       || [])   : [];
      p._phLog       = m ? (m._phLog       || [])   : [];
    ...
    state.plants.forEach(p => {
      p._notes=[]; p._feeds=[]; p._phLog=[]; p._ecLog=[];
      p._waterLog=[]; p._photos=[]; p._hardenStart=null;
      p._stage=p.status; p._lightHours=defaultLight(p);
    });
  }
}
...
async function fetchWeather() {
  ...
  try {
    ...
    document.getElementById('weather-panel').innerHTML =
      '<div style="padding:14px 18px;color:var(--text3);font-size:12px">Weather unavailable — check connection.</div>';
  }
}
...
function weatherNarrative(cur, daily) {
  ...
  const verdict = verdictColor
    ? '<strong style="color:' + verdictColor + '">' + verdictText + '</strong>'
    : verdictText;
...
function renderWeather(cur, daily) {
  ...
  const precip  = daily.precipitation_sum && daily.precipitation_sum[0] !== undefined
    ? daily.precipitation_sum[0].toFixed(2) : '0.00';
  ...
  const daylight = daily.daylight_duration
    ? (daily.daylight_duration[0] / 3600).toFixed(0) + 'h ' + Math.round((daily.daylight_duration[0] % 3600) / 60) + 'm' : '--';
  ...
  const lightHrs  = (() => {
    const [oh, om] = lightsOn.split(':').map(Number);
    const [fh, fm] = lightsOff.split(':').map(Number);
    let d = (fh * 60 + fm) - (oh * 60 + om);
    if (d < 0) d += 1440;
  ...
  summary.innerHTML = `
    ...
    <div class="wx-top">
      <div class="wx-current">
        ...
        <div class="wx-desc">${wxDesc(cur.weather_code || 3)}</div>
      ...
      <div class="wx-right">
        ...
        <div class="wx-moon">${mp.icon} Moon phase<br>${mp.name}</div>
      </div>
    </div>`;

  // ── Tell CSS how tall wx-summary actually is so sidebar/detail stick below it
  requestAnimationFrame(() => {
    const h = summary.offsetHeight;
    document.documentElement.style.setProperty('--wx-h', h + 'px');
  });

  // ── WEATHER PANEL — scrollable strips only
  const safe    = new Date(state.config.safePlanting || '2026-04-27');
  const today2  = new Date();
  const df      = d => Math.ceil((d - today2) / 86400000);
  const fmt     = d => d.toLocaleDateString('en-US', { month:'short', day:'numeric' });
  const lettH   = addDays(safe, -21);
  const tomH    = addDays(safe, -10);
  const pepH    = addDays(safe, -7);
  const lo0     = loToday;
  ...
  const forecastDays = daily.temperature_2m_max.map((hi, i) => {
    ...
    const badge = isFrz
      ? '<div class="wfd-badge freeze">FREEZE</div>'
      : isFrost
        ? '<div class="wfd-badge frost">FROST</div>'
        : '<div class="wfd-badge ok">OK</div>';
    const ic = icons[code] || '🌡';
...
function renderDetail() {
  ...
  if (!selectedId) {
    d.innerHTML = '<div class="empty-detail">← Select a plant to view details</div>';
    return;
  }
    ...
    <div id="detail-header">
      <div class="detail-header-inner">
        ...
        ${latestPhoto(p)
          ...
          : '<div class="detail-header-photo-empty">🌱</div>'}
      </div>
    </div>
...
function renderOverview(p, tc) {
  ...
  tc.innerHTML = `
    <div class="metrics-row">
      ...
      ${lph
        ? `<div class="metric"><div class="metric-label">Last pH</div><div class="metric-val ${lph.value >= 5.8 && lph.value <= 6.2 ? 'ok' : 'bad'}">${lph.value.toFixed(1)}</div></div>`
    ...
    <div class="card">
      <div class="card-label">Watering log <span class="count">${p._waterLog ? p._waterLog.length : 0}</span></div>
  ...
  tc.innerHTML = `
    ...
    <div class="metrics-row">
      <div class="metric"><div class="metric-label">Last pH</div>
        <div class="metric-val ${lph ? (phOk ? 'ok' : 'bad') : ''}">${lph ? lph.value.toFixed(1) : '—'}</div>
        <div class="metric-hint">Target 5.8–6.2</div></div>
      <div class="metric"><div class="metric-label">Last EC</div>
        <div class="metric-val">${lec ? lec.value.toFixed(2) : '—'}</div>
        <div class="metric-hint">Seedling 0.8–1.2</div></div>
      <div class="metric"><div class="metric-label">pH logs</div><div class="metric-val">${p._phLog.length}</div></div>
      <div class="metric"><div class="metric-label">Bucket</div><div class="metric-val sm">${p.bucket || '—'}</div></div>
    </div>
    <div class="two-col">
    ...
    ${dwcLogs.length ? `<div class="card"><div class="card-label">DWC session logs (${dwcLogs.length})</div>
      <ul class="log-list">
        ${[...dwcLogs].reverse().slice(0, 6).map(l => `<li>
          <div class="log-text"><strong>${l.date}</strong> · pH ${l.ph || '—'} · EC ${l.ec || '—'} · ${l.waterTemp || '—'}°F water
...
function renderGrowLog(p, tc) {
  ...
  tc.innerHTML = `
    ...
    <div class="card"><div class="card-label">Log history (${plantLogs.length} entries)</div>
      ${plantLogs.length ? `<ul class="log-list">${[...plantLogs].reverse().map(l => `<li>
        <div class="log-text">
      ...
      </li>`).join('')}</ul>` : '<div class="empty-msg">No grow logs yet.</div>'}
    </div>`;
}
...
function renderFeed(p, tc) {
  tc.innerHTML = `
    ...
    <div class="card"><div class="card-label">Feed history <span class="count">${p._feeds.length}</span></div>
      ${p._feeds.length ? `<ul class="log-list">${[...p._feeds].reverse().map(f => `<li>
        <span class="log-text">${f.text}</span><span class="log-date">${f.date}</span>
      </li>`).join('')}</ul>` : '<div class="empty-msg">No feeds logged yet.</div>'}
    </div>`;
}
...
function renderLight(p, tc) {
  ...
  tc.innerHTML = `
    ...
    <div class="card"><div class="card-label">Your setup</div>
      <ul class="log-list">
        ${(state.config.equipment && state.config.equipment.lights ? state.config.equipment.lights : []).map(l => `<li><span class="log-text">${l}</span></li>`).join('')}
      </ul>
    </div>`;
}
